...
authenticationController.post(
  "/register",
  body("username").isLength({min:5}).withMessage("Username and password must be 5 characters long!").isAlphanumeric("en-GB"),
  body("password").isLength({min:5}).withMessage("Username and password must be 5 characters long!").isAlphanumeric("en-GB"),
  async (req, res) => {
    try {
      const error = validationResult(req).errors;
      if (error.length > 0) {
      ...
      if (req.body.password !== req.body.rePassword) {
        throw newErrorWithFields("Passwords must match!", "password", "rePassword");
      }
